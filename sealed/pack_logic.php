<?php
	...
	function getCardList($conn) {
		...
		$sqlCommand = "SELECT `series` AS `setId`, `id` AS `setCardId`, `name`, `color` AS `inkType`, `rarity`, `image` FROM `lorcana_cards` ORDER BY RAND();";
...
function getRandomCardFromSet($cardsBySet, $setNum, $rarity, $inkType = null) {
	...
	function checkPackForDuplicateCards($boosterPack, $cardToCheck) {
		...
		foreach ($boosterPack -> cardsInBooster as $cardInPack) {
			...
			if ($cardInPack -> setId === $cardToCheck -> setId && $cardInPack -> setCardId === $cardToCheck -> setCardId) {
	...
	function buildPackFromSet($cardsBySet, $cardRarity, $packSet, $packNum) {
		...
		//6 Commons Per Pack; 1 Per Ink Type
				$card = getRandomCardFromSet($cardsBySet, $packSet, $cardRarity -> common, 'Amber');
				$boosterPack -> addCardToBooster($card);
				$card = getRandomCardFromSet($cardsBySet, $packSet, $cardRarity -> common, 'Amethyst');
				$boosterPack -> addCardToBooster($card);
				$card = getRandomCardFromSet($cardsBySet, $packSet, $cardRarity -> common, 'Emerald');
				$boosterPack -> addCardToBooster($card);
				$card = getRandomCardFromSet($cardsBySet, $packSet, $cardRarity -> common, 'Ruby');
				$boosterPack -> addCardToBooster($card);
				$card = getRandomCardFromSet($cardsBySet, $packSet, $cardRarity -> common, 'Sapphire');
				$boosterPack -> addCardToBooster($card);
				$card = getRandomCardFromSet($cardsBySet, $packSet, $cardRarity -> common, 'Steel');
				$boosterPack -> addCardToBooster($card);

		//3 Uncommons Per Pack
			for ($i = 1; $i <= 3; $i++) {
				do {
					$card = getRandomCardFromSet($cardsBySet, $packSet, $cardRarity -> uncommon);
		...
		//Slot 10:
			...
			for ($i = 1; $i <= 5; $i++) {
				for ($j = 1; $j <= count($cardsBySet -> setsByRarity[$packSet] -> rarities[$cardRarity -> rare]); $j++) {
					array_push($slot10Cards, $cardRarity -> rare);
				}
			}
			for ($i = 1; $i <= 3; $i++) {
				for ($j = 1; $j <= count($cardsBySet -> setsByRarity[$packSet] -> rarities[$cardRarity -> superRare]); $j++) {
					array_push($slot10Cards, $cardRarity -> superRare);
				}
			}
			for ($j = 1; $j <= count($cardsBySet -> setsByRarity[$packSet] -> rarities[$cardRarity -> legendary]); $j++) {
